...
def update_count_subgraph(count_subgraph, unlabeled_dataset_label, labeled_dataset_label, subgraph, radius) : 
    ...
    for i in unlabeled_index : 
        ...
        for j in i_subgraph : 
            count[labeled_dataset_label[j][0]-1] += 1
...
if __name__ == "__main__":
    ...
    #for num_subgraph in range(0,10) : 
    #    dist_class, adj_dist, classified_subgraph_index, pseudo_class_label = adjacency_subgraph(labeled_dataset, labeled_dataset_label, radius, CAE, num_subgraph)
    #    f_classification = first_classification(classified_subgraph_index, pseudo_class_label, subgraph, density_subgraph, 0, args.max_density)
    #    sc1_num_classification, sc1_score, sc1_dic_score = check_performance(f_classification,original_label)
    #    log(dest_dir_name, episode_id, "CS1", labeled_dataset_label, sc1_num_classification, args.max_density, num_subgraph, sc1_score)
    ...
    # 여기서 부턴 CNN 모델 학습!  
    neural_1 = MNIST_BN_32_64_256(26).to(device)
