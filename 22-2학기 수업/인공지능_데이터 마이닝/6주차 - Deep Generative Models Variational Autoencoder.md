...
- 기존의 variational inference에서는 데이터셋이 주어지지 않았을 때 만들어진 수식들이다. 
...
- **해결해야 하는 문제 : 어떻게 inference를 Neural Network로 훈련시킬 수 있는 구조를 만들 수 있을까?**
  ...
  > **어떻게 하면 Stochastic 한 상황에서 Neural Network를 학습시킬 수 있을 것인가?** 
  > 
  > Distribution 안에 넣은 Neural Network를 어떻게 계산할 것인가? 
  > 
  ...
  - **핵심은 "reparametrization Trick(또는 Stochastic Gradient Variational Bayes)"이다.**     
    ...
    > 
    >  이 두개의 lim은 특정 조건을 만족하지 않는 이상 서로 위치를 바꿀 수 없다.   바꿀 수 없다면 우리는 ELBO 식에 Differentiation이 가능해진다.  
    > 
    > 특정 조건이란? 
    > 
    > 이 조건을 만족시키지 못한다면 그 다음으로 나아갈 수가 없음 
    > 
    > Gaussian distribution에서는 특정조건을 충족하여 ELBO를 계산할 수 있음
    
    > Neural Network의 Chain rule의 관점에서 볼 때, Stochastic한 h의 값을 update 해야하는 deterministic value와 update를 하지 않아도 되는 Stocastic Value로 구분하자. 
    > 
    > ![](./picture/6-6.png)
    > 
    > ![](./picture/6-5.png)
...
>  계산을 줄이기 위해서 analytic 하게 풀이함. 자세한 풀이 과정은 교수님의 유튜브 참고하기 
> 
...
- Support Vector Machine(SVM) 
  ...
  - Kernel trick 까지 적용될 때 많은 의미를 가짐 
    ...
    > Kernel trick : Kernel trick은 Vector의 Basis를 확장하는 것을 모델링 하지 않고 Inner product를 계산하는 것. 원 정보를 담고 있는 Vector의 Basis를 Scalability 하게 등 다양하게 확장시킬 수 있음. 이때 Kernel trick을 통해 모델 확장 간의 계산양을 늘리지 않고도 원하는 목표를 이룰 수 있다는 것
