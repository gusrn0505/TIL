...
- KKT 조건에 따라서 목적함수의 제약식 부분은 아래 조건을 만족해야 함. 
  ...
  > SVM - Hard margin 때 증명했던 것처럼 Support vector 위에 있는 점들만 고려해주면 됨. 
...
- 목적 : Margin을 최대화하고, training error를 최소화하는 최소의 Hyperplane을 찾는 것 
...
- Mapping function $\phi(x)$을 도입함으로써 저차원의 input 벡터를 고차원으로 매핑한다. 
  ...
  - 적절한 Mapping을 통해서 Margin을 최대화하면서, 
...
- ##### 커널 함수의 종류
  ...
  - Gaussian (RBF) : $K(x,y) = exp(-\frac{||x-y||^2}{2\sigma^2}), \sigma \neq 0 $
...
- **RBF Kernel의 $\sigma$ 값, Cost와 경계 복잡도의 관계**
  ...
  - $\sigma$ 는 데이터 표준분산 값을 의미. ( <=> kernel 함수의 width를 의미함)
...
- SVR의 목적도 1) 에러를 최소화하면서, 2) 함수의 단순함(flatness)을 추구하는 것이다.
  ...
  - ex) $\epsilon - SVR $ 
    ...
    - 조치 2) Loss function으로 hinge loss를 선택하여, 예외에 덜 민감하게 반응 
      ...
      > 
      >  *기존에는 Loss function으로 Squared loss를 사용. 양 끝의 값을 가질수록 민감하게 반응했음*
...
- 목적 : 두 클래스를 보다 잘 구분해낼 수 있는 Projection line을 찾는 것 
  ...
  <=> Projction 했을 때 1) 클래스 간 거리는 최대화하면서, 2) 클래스 내 거리는 최소화하는 것 
...
4. **$J(w)$을 최대로 만드는 w의 값 찾기**
   ...
   - $S_Bw$ 는 항상 $(m_2-m_1)$ 방향을 지닌 벡터임 <mark>(식 2)</mark>
     
     > $S_Bw = (m_2-m_1)(m_2-m_1)^Tw$ = $(m_2 - m_1)$ * $\beta$ (s.t. $\beta$ 는 스칼라)
...
- <mark>$w^TS_W^\phi w$ 와 $w^TS_Bw$ 값을 모두 Kernel 함수의 형태로 변경 가능 </mark>
  ...
  > 
  > - $w = \sum_{n=1}^N \alpha_n \phi(x_n) , \alpha \in R$ (식 2)
  
  > 식 2 우측에 $m_i^\phi$ 를 곱함으로써 아래의 식을 도출할 수 있다.
  > 
  > ![](.\picture/2-50.png)
...
- $J(w)$ 의 식을 $J(\alpha)$ 식으로 변경하여 $\alpha$ 에 대한 식으로 정리하기 
  ...
  <img src=".\picture/2-53.png" title="" alt="" width="147"> =>  <img src=".\picture/2-54.png" title="" alt="" width="271">
...
2. **공분산 식을 통해 분산을 최대화하는 조건 탐색**
   ...
   - PCA에서 동일하게 최대한 $\lambda_k$의 클수록 전체 분산은 보전됨. 따라서 $\lambda_k$ 에 대응하는<mark> $v_k$ 에 대한 식을 찾는 것이 목표</mark>임. 
     
     <img src=".\picture/2-66.png" title="" alt="" width="331"> (식 3)
...
3. **식 2과 식 4에서 $v_k$ 를 $\phi(x)$의 선형결합으로 표현하여 공분산 식을 재정리.**
