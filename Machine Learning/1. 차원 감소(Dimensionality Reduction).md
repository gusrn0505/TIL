...
1. MDS에서 기술된 "거리 정보를 유지하는 축을 찾는 방법"이 어떻게 고차원에서 저차원으로 임베딩 간, S-Stress의 값을 최소화한다 할 수 있는지 확인 필요. 

2. LLE의 목적이 정확히 무엇인지 확인 필요. LLE 전개과정에 대한 이해 필요 
   
   - PCA는 분산 최대화, MDS는 거리 관계 유지 
   
   - LLE의 목적은 MDS와 유사한 것으로 판단되나 확실하지 않음. 
...
- 강의에서 기술된 과정은 **거리 정보를 보존하는 축을 찾는 방법**으로 기술되었는데, 위의 방식이 <mark>S-Stress를 최소화하는 방법인지 추가 확인 필요</mark>.
...
- ![](./picture/1-8.png)
  ...
  - 1) PCA / MDS 를 못 쓰는 이유 
       ...
       - PCA 와 MDS는 다차원 내에 최소거리를 측정함. 이를 그림 A의 파란색 점선과 동일함. 하지만 이는 주어진 데이터 공간에 적합하지 않음. 
...
##### 4. LLE(Locally Linear Embedding) 의의 <mark>(이해도 낮음- 고민 더 필요)</mark>

- 고차원 공간에서 데이터들 사이의 <u>선형적 구조</u>를 보존하면서 저차원으로 임베딩하는 것
  
  - Q. 여기서 선형적 구조라는 것은 Distance Matrix를 의미하나?  어떠한 목적(ex- 분산 최대 보존, 거리 관계 보존)에 초점을 둔 것인지? 
