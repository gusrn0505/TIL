...
- **전제** / **가정( - Gaussian Mixture Model)**
  ...
  - 각 레이블 별 입력값들을 생성하는 매커니즘이 있을 것이다. 
...
- **방향성**
  ...
  - **Unlabel 데이터의 y값을 찾기 위해 EM 알고리즘을 도입한다.** 
    
    - 1)label 데이터를 통해 지도 학습을 통해 모델을 만들고, Unlabel 데이터를 예측하여 신뢰도가 높은 값들을 label 데이터로 추가한다 (E-Step)  
      
      - 목표 : $p(X_l, y_l, X_u, y_u|\theta)$ 를 아는 것 
      
      > $\sum_{y_u} p(X_l, y_l, X_u, y_u|\theta) = p(X_l, y_l, X_u|\theta)$
      > 
      > $p(X_l, y_l|\theta) = \sum_{i=1}^l logp(y_i|\theta)p(x_i|y_i, \theta)$ 
    
    - 2)추가된 데이터를 포함하여 label 데이터에 최대우도법을 적용하여 $\theta$를 최신화한다.  (M-step)
    
    - 3)모든 데이터가 label 될 때까지 EM Step을 반복한다.  
