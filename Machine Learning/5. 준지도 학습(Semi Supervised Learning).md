...
- **전제**
  ...
  - <mark>가정 : 비슷한 샘플은 비슷한 출력값을 가진다   </mark>
    ...
    - 주로, 클러스터 가설 / 매니폴드 가설을 많이 채택한다. 
      
      - 클러스터 가설 : 데이터에 클러스터링 구조(밀집 구조)가 있다고 가정하고 같은 클러스터링에 속한 샘플을 동일한 클래스로 배분하는 방식 
      
      - 매니폴드 가설 : 데이터의 분포가 메니폴드 구조로 되어 있다고 가정하며, 이웃되는 샘플들이 비슷한 출력값을 가진다고 가정하는 방식 
...
- 서로 다른 데이터를 각자 학습한 후, 신뢰도가 높은 방식으로 그렇지 않은 방식을 재학습 시킴
  ...
  - 이론적으로 두개의 데이터가 독립적일 때, Unlabel 데이터를 기반으로 Co-training을 통해 학습기의 성능을 임의의 정도까지 향상할 수 있음이 증명되었음. 
    ...
    > 이거 왠지 앙상블 학습의 영역인 것 같은데. 
  - 단, 두 데이터가 완전히 상호배반이 아닐 수 있음. 그래도 여전히 효과적으로 약한 분류기의 성능을 향상시킬 수 있음이 증명됨. 
  - 후속 이론 연구를 통해 사실 상 단일 데이터로도 약한 학습기 사이에서 차이가 있어 다양성이 보장될 경우 일반화 성능을 향상시킬 수 있음 (앙상블 학습 )
  - 
...
- 서포트 벡터 머신의 SSL 에 맞춘 확장 버전 
  
  <img src="./picture/5-1.png" title="" alt="" width="313">
  
  - 기존 서포트 벡터 머신이 마진을 최대화하는 분할 초평면을 목표로 잡는 것에 비해 
  
  - 준지도 SVM에선 데이터의 저밀도 구역을 통과하는 분할 초평면을 찾기가 목적임.

###### 주요 방식 - TSVM

- 목표 : $D_u$ 샘플을 위해 예측 레이블 $\hat y = (\hat y_{l+1}, ..., + \hat y_{l+u}, \hat y_i \in (-1, +1))$ 을 찾는 것 
  ...
  > > 이때 $C_l, C_u$ 는 레이블된 샘플과 언레이블된 샘플의 중요도의 밸런스를 맞추는 절출 파라미터임.  
...
- **과정(ex - TSVM)**
  ...
  - 1) 지도학습 방식으로 label 샘플만을 통해 모델 학습 
    ...
    3) 추가/변형된 label 데이터를 포함하여 다시 SVM 모델을 학습함. 
    
    4) 이때 초기 SVM 모델과 레이블 분류가 다르거나, 오류 확률이 높은 unlabel 데이터를 찾아 레이블을 바꿔줌.   
    
    5) 3~4번 과정을 반복하여 정확도를 올려가며, 점차 $C_u$를 크게 만들어 최적화를 진행함 이는 $C_u = C_l$ 이 될떄까지 진행함.  
...
3. 계산량이 매우 많다. 
   ...
   - 샘플 수가 O(m)이라면 아록리즘에 관한 행렬 규모는 $O(m^2)$가 됨. 

###### Graph - based SSL 추가 단점
...
- 위의 정보들을 기반으로 클러스터링 기반의 알고리즘을 도입함
...
- **모델의 종류**
  ...
  - Generative Model (생성 모델) : 결과 값을 토대로 입력 값을 예측하는 모델. 입력 값의 분포 확률을 파악하는 것이 목적임  
    ...
    > $p(v,h) = p(v|h) p(h)$ : h라는 레이블이 주어졌을 때 v라는 설명 변수는 어떤 확률 분포를 띄는가? 
  
  - Discriminative model (판별 모델) : 입력 값을 토대로 결과 값을 예측하는 모델  
    
    > $p(h|v)$ : v라는 입력값이 나왔을 때 h label이 나올 확률 
...
- **전제** / **가정( - Gaussian Mixture Model)**
  ...
  - <mark>각 레이블 별 입력값들을 생성하는 매커니즘이 있을 것</mark>이다. 
  ...
  - 주로 가우시안 분포를 가정하나, 혼합 전문가 모델 또는 나이브 베이즈 모델 등도 적용한다. 
    ...
    > 
    > $f(x) \in Y$ 로 특정 샘플 x에 대해 label을 예측하는 모델이라 하고, $\theta$ 를 각 레이블 마다의 가우시안 분포들의 파라미터 집합이라고 할 때 아래를가 성립한다. 
    > 
    > - $f(x) = argmax_{j\in Y} p(y=j|x)$
    > 
    > - <=> $argmax_{y\in Y} \sum_{i=1}^N p(y=j, \theta = i|x)$
    > 
    > - <=> $argmax_{y \in Y} \sum_{i=1}^N p(y=j| \theta = i,x) * p(\theta = i|x) $
    >   
...
- **방향성**
  ...
  - **목표 : 최대 우도법을 적용하여 $p(X_l, y_l, X_u|\theta)$ 값을 최대화하는 $\theta$ 를 찾는것.**
    ...
    > 
    > > 이진 분류 상황으로, y의 값은 1과 2만 있다. 
    
    > > since $p(X_l, y_l|\theta) = \sum_{i=1}^l logp(y_i|\theta)p(x_i|y_i, \theta)$, 하이라이트 친 부분이 Unlabel data로 부터 나온 항목이다. 
    
    - 이때, 하이라이트 된 부분을 통해 Unlabel 데이터의 y값의 확률 값을 알아야 목표를 이룰 수 있음을 알 수 있다. 
  
  - **Gaussian Mixture Model의 파라미터 계산은 EM 알고리즘을 통해 구한다 .** 
    
    - E-Step : 현재 label 데이터로 구한 파라미터에 기반에 레이블이 없는 샘플 $x_j$가 각 가우스 혼합 성분에 속할 확률을 계산함   
...
- GMM을 통한 Generative 모델 장단점 
  ...
  - 장점 : 모델이 거의 정확하다면 매우 효율적일 수 있다. (계산량 관점인가?)
...
- **Variational Lower Bound** 
  ...
  - Generative 확률 모델 $p(X|\theta)$ 는 $\theta$ 값을 안다고 가정했을 때,  어떻게 X의 분포를 따르는 샘플을 생성할지 보여준다. 
    
    ![](./picture/5-20.png)
    
    > 분모의 X와 분자의 x는 동일해야 위의 식이 성립함. 
    
    > $p(\theta|X) : $ 사후확률(Posterior). X 데이터가 주어졌을 때의 $\theta$ 가 나올 확률 
    > 
    > $p(x|\theta) :$ 가능성(likelihood). $\theta$ 가 주어졌을 때 데이터 X가 생성될 확률 
    > 
  ...
  - **목표는 사후확률 $p(\theta|X)$ 을 찾는 것**이다. 
    ...
    - 단, <u>지금으로썬 p(X)의 값을 구할 수 없다</u>.(why?)
  
  - 해결책으로 $p(\theta|X)$를  아래 조건을 만족하는 $Q(\theta|X)$ 분포를 찾아 근사시킨다. 
    
    - $Q(\theta|X)$ 의 형태를 가정할 수 있어 계산과정이 상대적으로 빨라야 하며, 
    
    - $Q(\theta|X)$ 는 $P(\theta|X)$ 와 유사해야 한다. 
      
      - 이때 "유사성(closeness)" 정도를 측정하고자 KL divergence를 적용한다. 
    ...
    - 이 때, KL-divergnce 식을 정리함으로써 아래의 부등식이 나온다. 
...
- **SSL에 사용된 초기 VAE(Variational Bayes) - M1 모델**
  ...
  - 1단계 : label 데이터와 unlabel 데이터의 Input data(X)만 모아 latent variable(Z) 를 만든다. 
  ...
  - 2단계 : Latent variable Z와 label 값 Y를 묶어 지도학습을 통해 모델을 학습시킨다. 
...
- **VAE의 확장 - M2 Model** 
  ...
  - M1 모델에선 label 데이터 y값을 모델 학습 간 사용하지 않음. 이에 M2 모델에선  label 데이터 y값을 고려함 
    ...
    > 
    > - unlable 데이터는 그렇지 않아 latent variable Z 값을 통해 y 값을 추론해야함.  
...
- 일반 이미지에 노이즈를 섞을 경우 사람은 영향을 받지 않지만, 기계는 오류를 범하는 경우가 생김. 
  
  ![](./picture/5-29.png)

- Consistency Training 이란 모델 예측을 규제함으로써 작은 노이즈에 강건하게 만드는 것이다. 
  
  - augmentation 과정을 거치면 세부 정보가 바뀔 수 있음. 이는 noise가 추가된 것과 유사함.
  
  ![](./picture/5-30.png)
  
  - Augmentation 과정을 거친 것과 아닌 것의 결과가 Consistent 해야한다.
  
  > $\theta_s$ : Supervised 모델의 파라미터
  > 
  > $\theta_a$ : Augmentation 모델의 파라미터

- UDA에선 일정한 augmentation 방식을 적용함으로써 SSL간 성능을 향상시킬 수 있음을 보인다. 
