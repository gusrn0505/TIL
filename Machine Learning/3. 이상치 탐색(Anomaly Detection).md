...
- **방향성**
  ...
  - 임의의 변수의 임의의 특성 value로 구분하며, 고립(Isolation)을 원하는 객체가 속하는 방향의 데이터만 유지한다.  
    
    > 고립(Isolation) : 경계선 안에 유일하게 자신의 사례만 포함되는 경우  
  
  - 랜덤으로 변수와 속성값을 선택한다고 해도 수많은 사례를 뽑아 평균치를 구하면, 정상 사례를 고립시킬 때가 Minority 사례를 선택할 때보다 Split 횟수를 많이 가진다. 
    
    - Split 횟수는 Abnormal score와 반비례할 것이라 판단 
    
    ![](./picture/3-48.jpg)

- **의의** 
  
  - 간단한 방식으로 학습 계산량이 다른 방법에 비해 앞도적으로 적다. 
