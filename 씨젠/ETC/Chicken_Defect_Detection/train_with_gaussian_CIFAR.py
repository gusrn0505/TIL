...
#import Utility_nothing as U 
import Utility_gaussian_CIFAR as U

#if args.FGSM:
    #import Utility_FGSM as U
#    import Utility_practice_CIFAR as U
...
def main_worker(args): 
    ...
    air_idx = train_dataset.class_to_idx['airplane']
    auto_idx = train_dataset.class_to_idx['automobile']
    
    air_indice = []; auto_indice = []
    for i in range(len(train_dataset)) : 
        current_class = train_dataset[i][1]
        if current_class == air_idx : 
            air_indice.append(i)
        elif current_class == auto_idx : 
            auto_indice.append(i)

    import random
    random_idx = random.sample(auto_indice, 2500)
    len(random_idx)

    new_train_dataset = Subset(train_dataset, air_indice+auto_indice)
    new_pretrain_dataset = Subset(train_dataset, air_indice)

    air_idx = test_dataset.class_to_idx['airplane']
    auto_idx = test_dataset.class_to_idx['automobile']

    air_indice = []; auto_indice = []

    for i in range(len(test_dataset)) : 
        current_class = test_dataset[i][1]
        if current_class == air_idx : 
            air_indice.append(i)
        elif current_class == auto_idx : 
            auto_indice.append(i)
    ...
    import random
    random_idx = random.sample(auto_indice, 500)
    len(random_idx)

    new_test_dataset = Subset(test_dataset, air_indice+random_idx)
    
    train_loader = torch.utils.data.DataLoader(new_train_dataset, batch_size=args.batch_size,shuffle=True,num_workers=2)
    test_loader = torch.utils.data.DataLoader(new_test_dataset, batch_size=args.batch_size,shuffle=True,num_workers=2)
    ...
    for epoch in range(args.start_epoch, args.epochs):  # start_epoch 
        ...
        val_loss, val_recall, val_best_recall_threshold, val_auc, val_auprc=U.Validation(test_loader, models, criterions, device=device, log=log, args=args)
