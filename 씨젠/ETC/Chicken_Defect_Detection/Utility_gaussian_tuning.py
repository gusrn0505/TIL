...
# Train
def Train(train_loader, models, criterions, optimizers, mu, sigma, device=None, n_trial_DC=5, log=None, args=None):
    EPS = 1e-15
    ...
    ## generate noise
    import numpy as np
    noise = np.random.normal(mu,sigma,(3,448,448))
    # train loader
        ...
        if sum(target==0)>1:
            ...
            with torch.cuda.amp.autocast():
                ...
                for i, data in enumerate(normal) : 
                    perturbed = data.detach().cpu()+noise
